...
if DENY_ROLES is None:
    logger.info(
        "DENY_ROLES not set. Will check request headers %s and %s.",
        DO_CHECK_ROLES_HEADER,
        DONT_CHECK_ROLES_HEADER,
    )
elif DENY_ROLES == 0:
    DENY_ROLES = False
    logger.info("DENY_ROLES set to zero. Will NOT deny access based on roles.")
else:
    logger.info("DENY_ROLES set to %d. Will deny access based on roles.", DENY_ROLES)
    DENY_ROLES = True
